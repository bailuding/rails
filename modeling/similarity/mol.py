...
# to clean up code and to match latest public implementations.
...
from modeling.similarity.mol_query_embeddings import MoLQueryEmbeddingsFn
...
class SoftmaxDropoutCombiner(torch.nn.Module):
    ...
    def forward(
        ...
        if self._keep_debug_info:
            return (self._softmax_dropout(gating_weights, tau) * x).sum(-1), {
...
class SwiGLU(torch.nn.Module):
    """
    SwiGLU as coined in https://arxiv.org/pdf/2002.05202.pdf
    """
...
class MoLGatingFn(torch.nn.Module):
    """
    v8 but makes user- and item- side gating inputs optional.
    """
...
class MoLSimilarity(torch.nn.Module):
    ...
    def get_query_component_embeddings(
        ...
        """
        return self._query_embeddings_fn(input_embeddings, aux_payloads)
    ...
    def forward(
        ...
        with torch.autocast(enabled=self._bf16_training, dtype=torch.bfloat16, device_type='cuda'):
            ...
            split_user_embeddings, aux_losses = self.get_query_component_embeddings(
                input_embeddings,
                aux_payloads=aux_payloads,
            )
            split_item_embeddings = self.get_item_component_embeddings(input_embeddings=item_proj_input)
            if B_prime == 1:
                logits = torch.einsum(
            ...
            return self._gating_fn(
                ...
                query_embeddings=input_embeddings,  # [B, D]
